...
public partial class PresetEditDialog : Window
{
    ...
    public PresetEditDialog(ClickPreset preset, string title)
    {
        ...
        if (preset.Button == ClickButton.Right) RbRight.IsChecked = true;
        else RbLeft.IsChecked = true;
        if (preset.Behavior == ClickBehavior.Hold) RbHold.IsChecked = true;
        else RbRapid.IsChecked = true;
        if (preset.TriggerMode == TriggerMode.HoldActive) RbTrigHold.IsChecked = true;
        else RbTrigToggle.IsChecked = true;
    ...
    private void BtnOk_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            if (string.IsNullOrEmpty(_preset.Name)) _preset.Name = "未命名";
